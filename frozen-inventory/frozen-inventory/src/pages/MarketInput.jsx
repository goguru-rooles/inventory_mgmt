...
function calcSold(given, returned, isPastWeek) {
  const g = given || 0
  // null/undefined → market not yet settled
  if (returned === null || returned === undefined) {
    return isPastWeek ? g : 0   // past week: assume all sold; current week: not done
  }
  // 0 → user explicitly entered 0, meaning everything sold (nothing returned)
  return Math.max(0, g - returned)
}
...
// ── Tiny stepper ───────────────────────────────────────────────────────────────
function MiniStepper({ label, value, onChange, accent, allowNull = false }) {
  const styles = {
  ...
  const isNull = allowNull && (value === null || value === undefined)
  const displayVal = isNull ? '' : value
  return (
    <div className="flex flex-col items-center gap-0.5">
      <span className={`text-[10px] font-bold uppercase ${s.label}`}>{label}</span>
      <div className="flex items-center gap-0.5">
        <button onPointerDown={() => {
          if (isNull) onChange(0)                       // blank → 0 means all sold
        ...
        <input
          ...
          onChange={e => {
            if (allowNull && e.target.value === '') { onChange(null); return }
        ...
        />
        <button onPointerDown={() => {
          if (isNull) onChange(1)                       // blank → 1 first return
...
function ItemCard({ item, itemSizes, txnBySize, onUpdate, index, isPastWeek, remainingBySize, allocatedBySize }) {
  ...
  for (const size of itemSizes) {
    ...
    totalSold  += calcSold(t.given || 0, t.returned ?? null, isPastWeek)
  }
  ...
  return (
    <div className={`animate-slide-up stagger-${Math.min(index + 1, 5)}
      ...
      {itemSizes.map((size, si) => {
        ...
        const returned = t.returned ?? null   // null = unsettled; 0 = all sold
          ...
          <div key={size.id} className="mb-1.5 last:mb-0">
            <div className="flex items-center gap-2">
              ...
              <MiniStepper label="Returned" value={returned}
                onChange={v => onUpdate(size.id, 'returned', v)} accent="orange" allowNull />
...
export default function MarketInput() {
  ...
  const saveRow = useCallback(async (marketId, itemId, sizeId, values) => {
    ...
    const { error } = await supabase.from('market_transactions').upsert({
      ...
      returned:   (values.returned === null || values.returned === undefined) ? null : values.returned,
  ...
  const getRemaining = useCallback((itemId, sizeId, isPastWeek) => {
    const startQty = startInv[itemId]?.[sizeId]
    if (startQty === undefined) return undefined  // no starting inventory set
    // on-hand = startQty - totalGiven + totalReturnedFromSettled + totalRestock
    // Items given to any market are immediately "out".
    // Items returned from settled markets (returned != null) come back.
    let totalGiven = 0, totalReturned = 0, totalRestock = 0
    for (const [, marketTxns] of Object.entries(txns)) {
      const t = marketTxns[itemId]?.[sizeId]
      if (!t) continue
      const given    = t.given   || 0
      const returned = t.returned ?? null
      totalGiven   += given
      totalRestock += t.restock || 0
      if (returned !== null) {
        totalReturned += returned                 // settled: returned items come back
      } else if (isPastWeek) {
        // Past week unsettled → assume all sold, nothing returned
      }
      // Current week unsettled → items are "out", already covered by totalGiven
    }
    return startQty - totalGiven + totalReturned + totalRestock
  }, [startInv, txns])

  // ── Given to OTHER markets (not the currently active one) ─────────────────
  const getAllocatedElsewhere = useCallback((itemId, sizeId, excludeMarketId) => {
    // Only count markets where returned is null (items still "out"/unsettled)
    let total = 0
    for (const [marketId, marketTxns] of Object.entries(txns)) {
      ...
      if (!t) continue
      const returned = t.returned ?? null
      if (returned === null && (t.given || 0) > 0) {
        total += t.given    // items are "out" at this market, not yet returned
      }
    }
    return total
  }, [txns])
...
<script>window.onload = () => { window.print(); }<\/script>
  ...
  // ── Market totals ─────────────────────────────────────────────────────────
  const getMarketTotals = (marketId, isPastWeek) => {
    let given = 0, returned = 0, sold = 0
    for (const item of items) {
      for (const size of getItemSizes(item)) {
        const t = txns[marketId]?.[item.id]?.[size.id] || {}
        const g = t.given || 0
        const r = t.returned ?? null
        given += g
        if (r !== null) returned += r
        sold  += calcSold(g, r, isPastWeek)
      }
    }
